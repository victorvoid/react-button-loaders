...
storiesOf('Buttons/Beetle', module)
  .add(
    ...
    withState({ btnState: '' })(({ store }) => (
      <Beetle state={store.state.btnState} onClick={() => handleClick(store)}>
        Save Changes
      </Beetle>
    ))
  )
  .add('loading', () => <Beetle state='loading'>Save Changes</Beetle>)
  .add('completed', () => <Beetle state='finished'>Save Changes</Beetle>)
